...
describe("Library app", () => {
  ...
  describe("Login", () => {
    test("login Link is shown when not logged in", async ({ page }) => {
      await expect(page.getByRole("Link", { name: "login" })).toBeVisible();
    });

    test("add book Link is not shown when not logged in", async ({ page }) => {
      await expect(
        page.getByRole("Link", { name: "add book" }),
      ).not.toBeVisible();
    });

    test("recommend Link is not shown when not logged in", async ({ page }) => {
      await expect(
        page.getByRole("Link", { name: "recommend" }),
      ).not.toBeVisible();
    });

    test("logout Link is not shown when not logged in", async ({ page }) => {
      await expect(
  ...
  describe("When logged in", () => {
    ...
    test("author birth year can be updated", async ({ page }) => {
      ...
      await page.locator("#born-input").fill("1965");
